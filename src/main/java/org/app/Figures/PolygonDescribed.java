...
public class PolygonDescribed extends Figure {
    private final double radius = 0;
    private Point2D.Double center;
    private double sides = 3;

    public PolygonDescribed() {
        this.sides = Math.max(sides, 3); // Минимум 3 стороны
        figureType = FigureType.POLYGON_DESC;
    ...
    public void updateBasicParameter(int index) {
        if (index == 0) {
            basicParameters.set(0, children.get(0).start.distance(children.get(1).start) / 2);
            updated.set(0, true);
        }
    }
    ...
    public void updateChildren() {
        ...
        for (int i = 0; i < children.size() - 1; i++) {
            ...
            double descRad = basicParameters.get(0) / Math.cos(Math.PI / basicParameters.get(1));
            basicParameters.set(0, descRad);
            children.get(i + 1).start.x = children.get(0).start.x + descRad * Math.cos(theta);
            children.get(i + 1).start.y = children.get(0).start.y - descRad * Math.sin(theta);
        }
    }
    ...
    public void drawMethod(Graphics2D g, CoordinatePlane plane) {
        if (isDrown()) {
            ...
            for (int i = 0; i < sides; i++) {
                double theta = startAngle + i * angleStep;
                double descRad = 2 * globalRadius / Math.cos(Math.PI / basicParameters.get(1));
                xPoints[i] = (int) (globalCenter.x + descRad * Math.cos(theta));
                yPoints[i] = (int) (globalCenter.y - descRad * Math.sin(theta));
            }
    ...
    public void writeToDXF(BufferedWriter writer) throws IOException {
        double incircleRadius = basicParameters.get(0);
        double radius = incircleRadius / Math.cos(Math.PI/sides);
    ...
    private java.util.List<Point2D.Double> calculateVertices(double radius) {
        ...
        for (int i = 0; i < sides; i++) {
            ...
            double descRad = 2 * radius / Math.cos(Math.PI / basicParameters.get(1)) / 10;
